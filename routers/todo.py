...
user_dependency = Annotated[dict, Depends(get_current_user)] # auth.py'den tokeni istiyoruz. Aşağıdaki işlemlerde o tokeni verify edip öyle işlemler yapıcaz

def redirect_to_login(): # login sayfasına yönlendirme fonksiyonu. Bunu aşağıda gerektiğinde(Genellikle giriş yapılmadığında veya tokenin invalid olduğunda) kullanacağız
    redirect_response = RedirectResponse(url="/auth/login-page", status_code=302)
    redirect_response.delete_cookie("access_token") #kullanıcının cookiesindeki tokeni bitmişse ya da saldırı vs yapıyorsa sil
